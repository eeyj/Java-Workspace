...
public class A_Variable {
	...
	public void printVariable() {
		...
		// 서로 다른 자료형 값 여러개를 출력문 하나에 동시에 +로 붙여줘야함.
		System.out.println("이유진 : " + (11000 * 4 * 5) + "원 입니다.");
		System.out.println("홍길동 : " + (11000 * 7 * 20) + "원 입니다.");
		System.out.println("고길동 : " + (11000 * 8 * 23) + "원 입니다.");
		
		System.out.println("변수 사용 후");
		int pay = 11000; // 자바에서 = 대입을 나타내는 연산자입니다. 같다는 == 사용함
		int time = 6;
		int day = 15;
	...
	public void declareVariable() {
		 ...
		 * [표현법] 자료형 변수명;
		 * 
		 * 자료형 : 어떤 값을 담아낼지, 어떤 크기에 값을 담아낼지에 따라서 변수의 크기 및 모양을 지정하는 부분
		 * 변수명 : 변수의 이름을 정하는 부분.
		 * 
		 * 주의할점
		 ...
		 *     
		 * 2. 같은 영역({})안에서는 동일한 변수명으로 선언이 불가하다.
		 * 
		...
		short sNum; //2byte
		int iNum; //4byte(약 -21억 ~ 21억) => 정수형중에 가장 대표적인 자료형(기본형)
		long lNum; //8byte
		
		// 2_2. 실수형
		float fNum; //4byte => 소수점아래 7자리까지 표현가능.
		double dNum; //8byte => 소수점아래 15자리까지 표현가능.
		...
		sNum = 2;
		iNum = 4;
		lNum = 8L; // long자료형임을 알려주기위해 소문자나 대문자 L을 표기해줌(L을 더 사용)
		
		fNum = 4.0f; //반드시 f를 붙여줘야한다.
		dNum = 8.0;
		
		ch = '웅'; //반드시 ''(작은따옴표)안에 넣어줘야함.
