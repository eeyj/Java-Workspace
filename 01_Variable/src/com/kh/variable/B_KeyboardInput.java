...
public class B_KeyboardInput {
	...
	public void inputTest1() {
		...
		String name = sc.nextLine(); //사용자가 엔터까지 입력할 때까지 대기 상태
		...
		int age = sc.nextInt(); //사용자가 입력한 값 중 정수형 값을 받을때 사용하는 메소드
		
		sc.nextLine(); // 버퍼에 남아있는 엔터(\n)값을 비워주기위해 실행 
		               // nextLine() 개행문자로부터 공백에 값을 모두 찾아서 출력하므로, nextInt()값 다음에 nextLine() 사용시 nextLine() 를 허공에 한번 사용하여 공백(\n)을 비워주어야한다. 
	...
	public void inputTest2() {
		...
		// 실수값을 뽑을때 => sc.netxDouble() / sc.nextFloat()
		...
		System.out.println(name+"님의 개인정보 : 성별="+gender+" , 나이는="+age+", 키="+height+"cm");
	 ...
	 *   sc.nextLine()메소드를 한번더 실행시켜줘 버퍼에 남아있는 엔터를 빼줘야한다.
	 *   
	 *   2) '문자'값을 입력받아야할 경우
