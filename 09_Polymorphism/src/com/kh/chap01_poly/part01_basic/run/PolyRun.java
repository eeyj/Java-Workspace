...
public class PolyRun {
	...
	public static void main(String[] args) {
		 ...
		 * 
		 * 1. 부모타입 레퍼런스 변수로 부모타입의 객체를 다루는 것 => 당연히 가능하다.
		 * 
		 */
		System.out.println("1. 부모타입의 레퍼런스변수로 부모객체를 다루는 경우");
		...
		Child1 c1 = new Child1(); // 기본생성자에 슈퍼생성자로 호출하는 구문이 항상있음. 따라서 부모객체사용가능
		c1.printChild1();
		c1.printParent(); //((Parent)c1).printParent();  -------> 자동으로 형변환 된 것
		// c1 레퍼런스로 Parent(부모), CHild1(자식) 에 있는 필드나 메서드 둘다 접근가능.
		// c1 레퍼런스 변수가 Parent에 있는 메소드에 접근시 자동으로 형변환된채 진행됨
		// 즉, 자식에서 부모클래스로 자동형변환된다.(업캐스팅) -> 작은것에서 큰것으로 형변환 된것과 같은 이치
		...
		//          Parent 라는 형변환이 자동으로 JVM이 반영 
		/*
		 * 양쪽에 자료형이 다름에도 불구하고 오류발생 x
		 * 왜냐하면 Parent형으로 '자동형변환'이 되고 있기 때문에
		 * 상속 구조에서는 클래스 형변환이 가능하다.
		 * 
		 */
		p2.printParent(); //p2라는 레퍼런스 변수로는 Parent에만 접근이 가능
		((Child1)p2).printChild1(); //단, 자식객체로 접근하고 싶을때 Child1으로 강제형변환 후 Child1에 접근가능
		...
		// 단, 다형성이 적용되면 부모타입 레퍼런스 변수로 모든 자식객체를 다 받아 줄 수 있다.
...
//			}
			...
			// 단, 오버라이딩을 이용하면 굳이 형변환도 안해도 됨.
			arr[i].print();		
			/*
			 *  * 동적바인딩 :	프로그램이 실행되기전에 자동으로 컴파일되면서 정적으로 현재 레퍼런스 변수의 메소드를 가리키
			 *  단, 참조하고 있는 자식클래스에 해당 메소드가 오버라이딩 되어 있다면
			 *  프로그램을 시작할때 동적으로 그 자식클래스의 오버라이딩된 메소드를 찾아서 실행하게됨
