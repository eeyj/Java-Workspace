...
public class Run {
	...
	public static void main(String[] args) {
		...
		//Sports s = new Sports();
		//추상클래스로 절대 생성 불가 => 미완성된 클래스이기 때문에
		
		
		Sports s; // 단, 객체생성만 안될뿐 레퍼런스 변수로는 사용가능.
		s = new Football();// 즉 다형성을 적용해서 자식객체를 받아주는 용도로는 사용이 가능.
		 ...
		 *   
		 *   단, 추상메소드가 굳이 없어도 추상클래스로 둘 수 있긴함.
		 *   언제? => 클래스가 아직 구체적으로 덜 구현된 상태인것으로 보이는 경우(개념적)
		 *        => 현재 이 클래스를 객체 생성이 불가하게끔 하고자 할떄(기술적)
		 *        
		 *        
		 *   - 객체 생성 불가 
