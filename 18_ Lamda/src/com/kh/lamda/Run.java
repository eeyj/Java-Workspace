...
public class Run {
	 ...
	 * 실제로는 익명구현객체(이름없는클래스)를 생성하며 내부에 내가 작성한 코드를 실행시키는 방식으로 구동됨.
	 * 
	 * 람다식의 장점 : 표현이 간결해지고 가독성이 좋다.
	 *              내부적으로 람다식이 활용된 메소드가 많다.
	 *        단점 : 람다식에대해 이해도가 부족하면 알아보기 힘듦
	 *        
	 * [표현법]
	 * 변수 변수명 = (매개변수) -> {구현코드};
	 * 
	...
	@FunctionalInterface //내부적으로 1개의 미구현 함수를 지닌 인터페이스(람다식으로 활용할 인터페이스)
	...
	public static void main(String[] args) {
		...
		lamda4 l4 = a -> a*a; // 매개변수가 하나면 () 생략가능, 코드가 단순 return문이 끝이면 {} 생략 가능
