...
public class ArrayForeach {
	...
	public static void main(String[] args) {
		 ...
		 * 배열 또는 컬렉션과 상용됨
		 * 배열 또는 컬렉션의 0번 인덱스부터 마지막 인덱스까지 순차적으로 접근할 목적일떄
		 * 
		 * 초기식 조건식 증감식 없음.
		...
		phones[1] = new Phone("갤럭시", "zplip4", "삼성", 1450000);
		...
		System.out.println("총 평균 : "+ (sum/phones.length) + "원");
		
		
		//사용자에게 구매하고자하는 폰의 시리즈를 입력받고
		//구매하고자하는 폰이 phones배열에 있으면 
		//"xx핸드폰의 가격은 xxx원입니다."
		//구매하고자하는 핸드폰이 없다면 "해당 핸드폰이 없습니다."
		...
		int count = 0;
		for(Phone phone : phones) {
			if(sp.equals(phone.getSeries())) {
				System.out.println(phone.getName()+" "+phone.getSeries()+"의 가격은 "+phone.getPrice()+"원 입니다.");
				count++;
				break;
			}
		}
