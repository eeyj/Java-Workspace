...
public class Server {
	...
	public void setverStrart() {
		...
		try {
			...
			while(true) {
				// 3) 접속요청이 오면 요청수락(accept())후 해당 클라이언트의 대한 소켓객체를 생성
				// 4) 클라이언트의 정보를 저장
				...
				PrintWriter pw = new PrintWriter(os, true); // 데이터를 출력할때 print(), println()메소드를 사용
				...
				while(true) {
					...
					if(message == null || "exit".equals(message)) {
						...
						pw.flush(); // 현재 스트림에 있는 데이터를 강제로 내보내는 메소드. close() 메소드 호출되면 자동으로 flush 호출된다.
					}
