...
public class ListRun {
	...
	public static void main(String[] args) {
		 ...
		 * 
		 * 프레임워크
		 * 효율적인 기능들이 이미 정의되어있는 툴(프레임)
		 * 
		 * 정리해보면
		 * 데이터들이 새로이 추가되거나, 삭제가 되거나, 수정이 되는 기능(알고리즘) 들이 이미 정의 되어있는 틀이 있다.
		 * == 컬렉션
		 * => 다양한 데이터들을 관리하고자 할 때 배열 가지고 충분히 사용했었음, 단 배열의 단점들을 보완한 것이 컬렉션.
		 * 
		 * 배열과 컬렉션의 차이점
		 * - 배열의 단점
		 * 1. 배열을 쓰고자 할때 먼저 크기를 지정해야함.
		 *    => 한번 지정된 크기는 변경이 불가
		 *       새로운 값을 추가하고자 할떄 크기가 오버될 경우 새로운 크기의 배열을 만들고 기존의 내용들을 복사해주는
		 *       코드를 직접 구현해야함.
		 ...
		 *            ex) ArrayList, Vector, LinkList => ArrayList가 제일 많이 쓰임(90%이상)
		 * Set 계열 => 담고자 하는 값(value)만 저장/ 값 저장시 순서 유지 x(index개념이 없음) / 중복값 허용 x
		 *            ex) HashSet, TreeSet
		 * Map 계열 => 키(key) + 담고자하는 값(value) 세트로 저장 / 값 저장시 순서 유지 x(index 개념 없음)
		 *            / key 중복 허용 x
		 ...
		 * 해당 컬렉션에 다양한 타입의 데이터 값들이 담길 수 있음.(Default E == Object)
		 * 
		...
		// 2. add(int index, E e) : 리스트에 전달되는 인덱스값 위치에 전달되는 e를 추가시켜주는 메소드
		list.add(1, new Music("바다의 왕자","박명수"));
		// 중간의 값 추가시 알아서 기존의 값들을 뒤로 땡겨주는 작업이 내부적으로 진행됨.
		...
		// 6. get(int index) : E => 리스트에 해당 index위치에 데이터를 반환해주는 메소드
		// 제네릭 설정 전
