...
public class Run {
	...
	public static void main(String[] args) {
		...
		hs1.add(new String("여러분"));
		//hs1.add(50); 제네릭 설정한 String 타입만 값 입력 가능
		
		System.out.println(hs1); // 중복 값 저장하지 않음. 저장 순서를 유지하지 않는다.
		...
		// 저장 순서 유지 안됨., 중복값은 저장이됨 -> 동일객체로 판단되지 않기 때문
		...
		// Object클래스에 잇는 hashCode() : 두 객체의 주소값을 기반으로서 해시코드값을 만들어서 반환하여 비교
		
		// 만약 위 2개의 메소드를 내가 원하는 형태로 바꿔서 중복저장이 안되게 하고 싶다면 오버라이딩을 해줘야함.
		
		// Student클래스에 equals()오버라이딩 : 새 필드값(name, age, score)이 일치하면 true, 아니면 false
		// Student클래스에 hashCode() 오버라이딩 : 새 필드값 기반으로 해서 해시코드값을 만들어서 반환
		
		System.out.println("=============================================================");
		
		
		// Set => 무작위로 객체가 저장되어있음. (index 개념이 없음) => 따라서 반복문 활용이 불가
		...
		while(it.hasNext()) { // .hasNext() : it에 next로 빼올 값이 있는가? 라고 물어보는 메소드 
			it.next();
