...
public class FileRun {
	...
	public static void main(String[] args) {
		...
		try {
			// 1. 별도의 경로 지정을 하지 않고 파일명(test.txt)만 제시해서 생성해보기 => 경로는 현재 디렉토리
			...
			// 4. 별도의 경로 지정 없이 폴더 생성 후 파일 생성하기 => 프로젝트 폴더 내에 생성될것.
		 ...
		 * 
		 * - 외부매체와의 직접적인 연결여부 : 
		 *   기반스트림 : 외부매체의 직접적으로 연결되는 통로
		 *   보조스트림 : 기반스트림만으로 부족한 성능을 향상시켜주는 용도의 스트림(직접적으로 외부매체와 연결할 수 없음, 즉 단독사용이 불가)
		 *             (보조해주는 기능 : 속도향상, 자료형 반환, 객체단위의 입출력 보조 등등)
		 *             단, 보조스트림은 단독적으로 사용이 불가능하므로 반드시 외부매체와 직접적으로 연결가능한 기반스트림과 함께 사용해야한다.
		 * 
		 * 
		 */
