...
public class B_ArrayCopy {
	...
	public void method3() {
		// 2. 새로운 배열을 생성한 후 System 클래스에서 arraycopy 메서드 이용
		
		int[] origin = {1,2,3,4,5};
		
		int[] copy = new int[10];
		
		//System.arraycopy(원본 배열명, 원본배열의 복사를 시작할 인덱스, 복사본 배열명, 복사본 배열의 복사될 시작 인덱스, 복사할 개수);
		...
		// 0 0 0 0 0 0 0 0 3 4
		//                 -> 마지막 인덱스를 벗어나서 복사를 실행(ArrayIndexOutOfBoundsException)
		// 인덱스를 벗어난 범위에서 복사는 불가능함.
	...
	public void method4() {
		 ...
		 * 복사할 것인지 세밀하게 지정가능/
		 * 
		 * 3. Array.copyOf 메소드 : 무조건 원본 배열의 0번 인덱스부터 내가 제시한
		 * 갯수만큼 복사 진행됨. 내가 제시한 길이가 원본배열보다 크다면 나머지에는 0 값으로
		 * 채워서 복사해줌.
		 * 
		 * 
		 * 
	...
	public void method5() {
		...
		// 복사본 배열 = 원본배열.clone();
		int[] copy = origin.clone(); //인덱스 직접 지정 x, 복사할 겍수 지정x
		...
		/*
		 * Arrays.toString(배열);
		 * -> 앞과 뒤에 [] 각 값마다 ,를 넣어서 하나의 문자열로 연이어서 출력하는 메서드
		 */
