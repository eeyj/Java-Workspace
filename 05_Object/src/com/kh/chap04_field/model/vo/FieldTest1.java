...
/*
 * 변수구분
 * ->전역변수 : 클래스 영역에 배로 선언하는 변수 -> 클래스 내에서면 어디서든 사용 가능
 * ->지역변수 : 클래스 영역내의 특정 영역({})에 선언한 변수 (선언한 영역에서만 사용가능)
 *            ex) 메소드 영영안, if문, for문 등등
 *            
 * 1. 전역변수(2가지로 구분)
 *  - 멤버변수 == 인스턴스 변수 == 필드
...
public class FieldTest1 { // FieldTest1 클래스 시작
	...
	public void test(int num) { //test메소드 시작
		...
		// 지역변수(메서드 영역 내에 생성.) => 지역변수에는 접근제한자 붙이지 않음.
		...
		// 매개변수 출력시 에러 안나는 이유
		// 해당 메서드(test)호출시 반드시 값을 넘겨줘야 호출이 가능하기 때문에 num에는 반드시 어떤값이든 초기화가 될것.
