...
public class Run {
	 ...
	 *    3) 그렇게 추려진 속성들을 가지고 내가 구현할 프로그램의 "실질적인 목적"에 맞춰서 불필요한 속성과 기능들을 제거하기
	 ...
	 *                   홍길동이라는 학생객체를 만들기 위해선 => String name1 = "홍길동";, int age1 = 20; double height1 = 
	 *                   민경민이라는 학생객체를 만들기 위해선 => String name2 = "민경민";, int age2 = 30; double height2 =
	 *                                               => String name3 = ...............................; 
	 *                   => 관리하고자하는 학생들이 늘어나면 늘어날 수록 생성해야할 갯수도 엄청나게 증가하게됨. 즉 곽리하기가 굉장히 힘듬
	 *                   
	 *               2) "배열"을 이용해서 프로그래밍을 하게 된다면 ?
	 *                  배열 : 하나의 자료형만 여러개의 값들을 보관할 수 있음(단, 동일한자료형일시)
	 *                  학생들의 이름을 보관하는 배열 선언 => String[] name = {"홍길동", "민경민", "김갑생"};
	 *                  학생등의 나이를 보관하는 배열 선언 => int[] age = {20, 30, 40};
	 *                  학생들의 키를 보관하는 배열 선언 => double[] height= {}
 	 *                  => 변수를 사용하는 것보다는 나아보임
 	 *                  => 하지만 관리하는 학생중 한명이 전학을 가게되는 경우 해당 인덱스에 있는 모든 키, 나이에 대한 정보도 제거를 해줘야함
 	 *                     그리고 새로운 학생이 들어오게 되는 경우 배열의 특성상 한번 크기를 지정하게 되면 변경하는게 불가능하므로 새로운
 	 *                     정보를 입력하기 위해 배열을 복사해서 새롭게 추가해야함, 즉 유지보수가 힘듬
 	 *               
 	 *               3) "클래스(구조체)"를 가지고 프로그래밍한다면?   
 	 *                  구조체 : 한번에 여러개의 자료형을 보관할 수 있는 배열같은 형태.
 	 *                         String값도 보관하고, int값도 보관하고,double값도 보관할 수 있는 나만의 자료형을 만들 수 있음
 	 *                  클래스 : 각 객체들의 속성 정보들을 담아내는 그릇과도 같은 존재, 보통 vo 패키지 안에 보관한다.
 	 *                  
	...
	public static void main(String[] args) {
		...
		// xxx님의 나이는 xx살이고, 키는 xxx.x cm입니다.
