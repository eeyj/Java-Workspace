...
public class MethodTest2 {
	 ...
	 * static 메소드
	 * [표현법]
	 * 접근제한자 static 예약어 반환형 메소드(매개변수) {
	 * 		수행내용
	 * 		return 반환값;
	 * }
	 * 
	 * - 호출시 객체 생성할 필요가 없음
	 * - 프로그램 시작시에 정적 메모리 영역에 메소드가 저장되어 있을것이기 때문.
	...
	public static int method4(int num1) {
		 ...
		 * 이때 static으로 선언된 필드(전역변수)도 함께 정적메모리 영역에 들어갈것.
		 * 하지만 static으로 선언되지 않은 전역변수는 static 메모리에 올라가는 시점에 사용이 불가능하므로 에러가 발생함.
		 */
